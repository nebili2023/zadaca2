...
if(isset($_GET["dan"]) && isset($_GET["mesec"]) && isset($_GET["godina"]))
{
	$dan = $dan_old = $_GET["dan"];
	$mesec = $mesec_old = $_GET["mesec"];
	$godina = $godina_old = $_GET["godina"];
	if($dan>0 && $dan<10) $dan = "0".$dan_old;
	if($mesec>0 && $mesec<10) $mesec = "0".$mesec_old;
	
	$datum = date($godina."-".$mesec."-".$dan);
	$print_datum = date($dan.".".$mesec.".".$godina);
}
else
{
	$datum = date("Y-m-d");
	$print_datum = date("d.m.Y");
	$dan_old = date("j");
	$mesec_old = date("n");
	$godina_old = date("Y");
	 ...
	 $denesen_datum = date("Y-m-d"); 
	 $denes = strtotime($denesen_datum); 
	 $izbran_datum = strtotime($datum);
	 if($izbran_datum >= $denes) 
	 	 $validen = "da"; 
		 else
		 $validen = "ne"; 
	...
	$svikorisnici = $korisnici->getAllDan($datum);
...
$nextdaydate  = date('j', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))+1, date("Y", strtotime($datum))));
$nextdaymonth  = date('n', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))+1, date("Y", strtotime($datum))));
$nextdayyear  = date('Y', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))+1, date("Y", strtotime($datum))));

$previousdaydate  = date('j', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))-1, date("Y", strtotime($datum))));
$previousdaymonth  = date('n', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))-1, date("Y", strtotime($datum))));
$previousdayyear  = date('Y', mktime(0, 0, 0, date("m", strtotime($datum))  , date("d", strtotime($datum))-1, date("Y", strtotime($datum))));
?>
...
<table cellspacing="0" align="center" cellpadding="0" border="0" width="700">
	<tr align="center">
    	<td valign="middle" align="left">
        	<a href="index.php?dan=<?php echo $previousdaydate ?>&mesec=<?php echo $previousdaymonth ?>&godina=<?php echo $previousdayyear ?>"><img src="../images/leftarr.png" height="30" style="border:none;padding-left:5px;"/></a>
        </td>
    <td valign="middle">
    	<h1><?php echo $print_datum; ?></h1></div>
    </td>
    <td valign="middle" align="right">
        	<a href="index.php?dan=<?php echo $nextdaydate ?>&mesec=<?php echo $nextdaymonth ?>&godina=<?php echo $nextdayyear ?>"><img src="../images/rightarr.png" height="30" style="border:none;padding-right:5px;"/> </a>
        </td>
   	</tr>
</table>
	...
	for($j=0;$j<14;$j++) {
		...
		else
		{
			...
			else
			{
				if($validen=="da")
				{
					echo '<tr height="30" class="'.$class.'" ><form action="index.php?dan='.$dan_old.'&mesec='.$mesec_old.'&godina='.$godina_old.'" method="POST"  onSubmit="return validate(this)">';
					echo '<td>'.$termini[$j].'</td>';
					...
					echo '<input type="hidden" id="korisnik_'.$j.'_hidden4" name="datum" value="'.$datum.'">';
					echo '</td>';
